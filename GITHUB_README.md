...
## 💰 Connecting to a Real Betting System

The three layers that turn this into a betting engine:

**Layer 1 — Find the edge**

```
bookmaker quotes 2.10 for Isner to win the set
→ implied probability = 1 / 2.10 = 0.476
→ model outputs p1_set_win = 0.55
→ edge = 0.55 - 0.476 = +7.4%   ← positive edge: bet Isner
```

**Layer 2 — Size the bet (Kelly Criterion)**

```
f* = edge / (odds - 1) = 0.074 / (2.10 - 1) = 6.7% of bankroll
```

Over many bets with a positive edge, Kelly maximises long-run bankroll growth.
Without a calibrated probability model, this calculation is impossible.

**Layer 3 — The latency moat**

In-play markets refresh every 1–3 seconds after a point ends. The bookmaker's line
lags the real match state. This pipeline prices the new probability in **0.073 ms** —
~40,000× before the market corrects. That time window is the entire business model.

> The model gives you a fair price; the bookmaker gives you a market price; the
> difference is your edge; Kelly tells you how much to bet; and the sub-millisecond
> pipeline ensures you act before the market corrects.
...
| Metric | Value |
|---|---|
| Per-point latency (mean) | ~0.009 ms |
| Per-point latency with recursion | ~0.073 ms |
| p99 latency | ~0.013 ms |
| Production budget | 200 ms |
| Margin under budget | **2,700×** |
| Training data | 18,249 matches / 2,820,681 points |
| ONNX vs XGBoost diff | < 1e-6 |
| Model accuracy | 63.2% (ROC AUC 0.545) |
| Hyperparameter trials | 20 (Optuna) |
...
│   ├── preprocessing/
│   │   ├── loader.py        # SackmannLoader — CSV discovery + match row iteration
│   │   ├── parser.py        # MatchParser — PBP string → GameState records
│   │   ├── validator.py     # ScoreValidator — parsed vs expected score check
│   │   ├── pipeline.py      # TrainingPipeline — orchestrates, saves CSVs
│   │   ├── models.py        # GameState dataclass
│   │   └── __main__.py      # CLI entry point
│   ├── training/
│   │   ├── config.py        # TrainingConfig dataclass
│   │   ├── data.py          # DataSplitter
│   │   ├── tuner.py         # HyperparameterTuner (Optuna, 20 trials)
│   │   ├── trainer.py       # XGBoostTrainer
│   │   ├── evaluator.py     # ModelEvaluator
│   │   ├── exporter.py      # OnnxExporter (verified, max diff < 1e-6)
│   │   ├── pipeline.py      # ModelTrainingPipeline
│   │   └── __main__.py      # CLI entry point
│   ├── ingestion/
│   │   ├── models.py        # MatchEvent, OddsUpdate dataclasses
│   │   ├── producer.py      # MatchEventProducer — CSV → typed event stream
│   │   ├── queue.py         # EventQueue — thread-safe FIFO + backpressure
│   │   ├── state.py         # GameStateManager — O(1) state + Carter–Pollard recursion
│   │   ├── engine.py        # InferenceEngine — stateless ONNX wrapper
│   │   ├── consumer.py      # EventConsumer, OddsPublisher
│   │   ├── pipeline.py      # IngestionConfig, IngestionPipeline
│   │   └── __main__.py      # CLI entry point
│   └── dashboard/
│       └── app.py           # Streamlit live-odds dashboard (~160 lines)
├── notebooks/
│   ├── 01_raw_data_to_training_set.ipynb
│   ├── 02_model_training.ipynb
│   ├── 03_data_ingestion_and_queues.ipynb
│   └── 04_streamlit_dashboard.ipynb
├── models/                  # model.onnx  (produced by training pipeline)
├── data/                    # Sackmann ATP CSVs + generated datasets
├── pyproject.toml
└── uv.lock
```
...
Each notebook is self-contained and imports from the `src` package — classes are not
re-defined inline. They form a progressive learning path:

| Notebook | Purpose |
|---|---|
| `01_raw_data_to_training_set` | CSV exploration, PBP parsing, feature engineering, dataset export |
| `02_model_training` | XGBoost training, Optuna tuning, ONNX export, latency benchmark |
| `03_data_ingestion_and_queues` | Event model, queue, state, inference, e2e demo, latency analysis |
| `04_streamlit_dashboard` | Dashboard design, session state lifecycle, streaming pattern |
